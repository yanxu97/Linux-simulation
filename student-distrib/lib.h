...
#define OFFSET 		0xAF		/* constant to track enter position */
#define BUF_SIZE	128			/* size of line buffer */


/* some constants */
#define CURPORT1  	0x0F
#define CURGGMF1	0x03D4
#define ENENEN		0xFF
#define THEFOURTH	0x03D5
#define THEFIFTH	0x0E		
#define EIGHT		8
#define TEST_V		36864
#define MAX_X		79
#define MAX_Y		24
...
/* Move the cursor position according to the input */
void update_cursor(uint16_t row, uint16_t col);
/* Scroll the video memory down by one row */
void scrolling();
/* Clear the keyboard buffer and set position to 0 */
void keyboard_buffer_reset();
 /* Delete one character from console */
void backspace();
/* Clear the console */
/* Put the cursor at top of console */
void reset();

void save_current_terminal(uint8_t from, uint8_t to);

/* helper functions for terminal switch */
int x_pos();
int y_pos();
void update_x(int x);
void update_y(int y);
